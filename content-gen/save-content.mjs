...
if (!channel || !type) {
  console.error("Usage: save-content.mjs <file.json|--json '{...}'> --channel <id> --type <type>");
  process.exit(1);
}
...
function extractJson(raw) {
  ...
  if (start !== -1) {
    ...
    for (let i = start; i < raw.length; i++) {
      ...
      if (inStr) continue;
      if (c === "{") depth++;
      else if (c === "}") { depth--; if (depth === 0) return raw.slice(start, i + 1).trim(); }
    }
  }
  return raw.trim();
}
...
if (!data || typeof data !== "object") {
  console.error(`❌ Failed to parse JSON for ${channel}/${type}`);
...
function assessQuality(data, type) {
  ...
  if (type === "question" && data.sections?.some((s) => s.type === "code" && s.content?.length > 30)) score++;
...
`).run(data.id, channel, type, JSON.stringify(data), qualityScore, status, agentId);
