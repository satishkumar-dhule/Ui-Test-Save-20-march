...
export default function App() {
  ...
  const [channelId, setChannelId] = useLocalStorage(
    "devprep:channelId",
    "javascript",
  );
  ...
  // Destructure stable callbacks so useEffect deps don't change each render
  const {
    trackSearchOpened,
    trackVisitedQuest,
  ...
  }, [isSearchOpen, trackSearchOpened]);
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return allQuestions.filter((q) => q.tags.some((t) => filter.includes(t)));
  }, [currentChannel, allQuestions]);

  const filteredFlashcards = useMemo(() => {
    const filter = currentChannel?.tagFilter;
    if (!filter) return allFlashcards;
    return allFlashcards.filter((f) => f.tags.some((t) => filter.includes(t)));
  ...
  const filteredCoding = useMemo(() => {
    ...
    return allCoding.filter(
      (c) =>
        c.tags.some((t) => filter.includes(t)) || c.channelId === channelId,
    );
  ...
  const handleOnboardingDone = useCallback(
    (ids: Set<string>) => {
      ...
      const techIds = selectedChs
        .filter((c) => c.type === "tech")
        .map((c) => c.id);
      const certIds = selectedChs
        .filter((c) => c.type === "cert")
        .map((c) => c.id);

      console.log(
        "Tracking onboarding - techIds:",
        techIds,
        "certIds:",
        certIds,
      );
      ...
      if (techIds.length > 0) {
        trackSignedUpTechChannels(techIds);
      }
      if (certIds.length > 0) {
        trackSignedUpCerts(certIds);
      }
    },
    [handleChannelSwitch, trackSignedUpTechChannels, trackSignedUpCerts],
  );
  ...
  }, [channelId, section, trackVisitedQuest]);
  ...
  const handleSearchResultSelect = useCallback(
    (result: SearchResult) => {
      ...
      trackSearchResultSelected(searchQuery, result.type, result.id);
      setSection(newSection);
      setIsSearchOpen(false);
      setSearchQuery("");
      setSearchResults([]);
    },
    [trackSearchResultSelected, searchQuery],
  );
      ...
      >
        ...
        <AppContent
          ...
          onCodingUpdate={(id, status) =>
            analytics.updateCodingProgress(id, channelId, status)
          }
