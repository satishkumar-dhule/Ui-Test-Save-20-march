const apiLogger = {
  warn: (msg: string, ctx?: unknown) => console.warn("[progressApi]", msg, ctx),
  error: (msg: string, ctx?: unknown) => console.error("[progressApi]", msg, ctx),
};
...
function lsSet(data: ProgressData) {
  try {
    ...
    apiLogger.error("Failed to save progress to localStorage", { error });
  }
}
...
async function fetchWithRetry(
  ...
  try {
    ...
    if (attempt < RETRY_CONFIG.maxRetries - 1) {
      ...
      apiLogger.warn(
        `Network request failed, retrying in ${delay}ms`,
        {
          attempt: attempt + 1,
    ...
    apiLogger.error("Network request failed after all retries", {
      url,
...
export const progressApi = {
  async load(channelId: string): Promise<ProgressData> {
    try {
      ...
      apiLogger.warn("Failed to load progress from API, using localStorage fallback", {
        channelId,
  ...
  async saveFlashcard(
    ...
    try {
      ...
      apiLogger.warn("Failed to sync flashcard progress to server", {
        channelId,
        cardId,
        status,
        error: error instanceof Error ? error.message : "Unknown error",
  ...
  async saveCoding(
    ...
    try {
      ...
      apiLogger.warn("Failed to sync coding progress to server", {
        channelId,
        challengeId,
        status,
        error: error instanceof Error ? error.message : "Unknown error",
  ...
  async saveExam(
    ...
    try {
      ...
      apiLogger.warn("Failed to sync exam progress to server", {
        channelId,
        examId,
        result,
        error: error instanceof Error ? error.message : "Unknown error",
  ...
  async saveVoice(
    ...
    try {
      ...
      apiLogger.warn("Failed to sync voice practice progress to server", {
        channelId,
        promptId,
        rating,
        error: error instanceof Error ? error.message : "Unknown error",
  ...
  async saveQA(
    ...
    try {
      ...
      apiLogger.warn("Failed to sync QA progress to server", {
        channelId,
        questionId,
        answered,
        correct,
