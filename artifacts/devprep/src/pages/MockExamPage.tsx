...
export function MockExamPage({
  ...
  const submitExam = useCallback(() => {
    setPhase("result");
    const durationMs =
      (45 * 60 - timeLeft + visibilityChangeRef.current) * 1000;
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
        ...
        <h3 className="text-lg font-semibold mb-2">
          No exam questions for this channel
        </h3>
        ...
        <div className="max-w-md mx-auto space-y-6 text-center">
          ...
          <div className="p-4 rounded-lg border border-border bg-card text-left space-y-3">
            {(["easy", "medium", "hard"] as const).map((d) => {
              ...
              const dPct =
                dQs.length > 0 ? Math.round((dCorrect / dQs.length) * 100) : 0;
              return (
                <div key={d}>
          ...
          {phase === "exam" && (
            <>
              ...
              <button
                ...
                style={{
                  borderColor: flagged.has(current)
                    ? "hsl(var(--chart-3))"
                    : "hsl(var(--border))",
                  color: flagged.has(current)
                    ? "hsl(var(--chart-3))"
                    : undefined,
                  background: flagged.has(current)
                    ? "hsl(var(--chart-3) / 0.1)"
                    : undefined,
                }}
              >
              ...
              {q?.choices.map((choice) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setAnswers((prev) => ({ ...prev, [current]: choice.id }))
                    }
