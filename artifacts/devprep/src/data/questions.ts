...
// 4: setTimeout (macrotask)`,
      },
      {
        type: 'eli5',
        content:
          "Imagine a **chef** (call stack) who can only cook one dish at a time. When an order takes long (like waiting for ingredients — that's async), the chef hands it to a **helper** (Web API). The helper signals back when ready. The chef checks a **VIP list** (microtasks / Promises) before the **regular queue** (macrotasks / setTimeout). So Promises always cut the line!",
      },
    ],
  },
  {
    ...
    sections: [
      {
        type: 'short',
        content:
          'A **closure** is a function that retains access to its **lexical scope** — variables from the outer function — even after the outer function has finished executing.\n\nClosures enable:\n- **Data encapsulation** — private variables\n- **Factory functions** — generate customized functions\n- **Memoization** — cache expensive computations\n- **Partial application / currying**',
      },
      {
        type: 'code',
        language: 'javascript',
  ...
  setTimeout(() => console.log(i), 100); // logs 0, 1, 2 ✅
      ...
      },
      {
        ...
        topics: [
          {
            title: 'Lexical Scope',
            description: 'How JavaScript determines variable access at write time, not run time',
            tag: 'javascript',
          },
          {
            title: 'IIFE Pattern',
            description: 'Immediately Invoked Function Expressions for module-like isolation',
            tag: 'javascript',
          },
          {
            title: 'Module Pattern',
            description: 'Use closures to create private/public APIs without ES modules',
            tag: 'javascript',
          },
        ],
      },
    ...
    sections: [
      {
        type: 'short',
        content:
          '**Hooks** let you use state and other React features in **function components**, replacing the need for class components.\n\nCore hooks:\n- `useState` — local component state\n- `useEffect` — side effects (data fetching, subscriptions)\n- `useContext` — consume React context\n- `useRef` — mutable ref / DOM access\n- `useMemo` / `useCallback` — performance optimization\n- `useReducer` — complex state logic',
      },
      {
        type: 'code',
        language: 'tsx',
  ...
  return (
    <div>
      ...
      },
      {
        type: 'eli5',
        content:
          "Before hooks, React components needed to be **class-based** to have memory (state). Hooks gave **function components** superpowers — `useState` is like a sticky note the component can read and update, `useEffect` is like saying 'do this whenever something changes'.",
      },
    ],
  },
  {
  ...
  return isLoggedIn ? <UserView /> : <GuestView />;
      ...
      },
      {
        ...
        topics: [
          {
            title: 'React Fiber',
            description: 'The reimplemented reconciler that enables concurrent features',
            tag: 'react',
          },
          {
            title: 'useMemo & useCallback',
            description: 'Prevent unnecessary reconciliation in child components',
            tag: 'react',
          },
          {
            title: 'React.memo',
...
function binarySearch(sorted, target) {
  ...
  return -1;
      ...
      },
      {
        type: 'eli5',
        content:
          "Imagine looking for a name in a phone book. **O(1)** is knowing exactly which page it's on. **O(n)** is checking every page one by one. **O(log n)** is opening to the middle, checking if your name is before/after, then halving the search each time — *much* faster for big books!",
      },
    ],
  },
  {
      ...
      },
      {
        ...
        topics: [
          ...
          },
          {
            ...
            description: 'Multiplexing, QUIC, and modern protocol improvements',
            tag: 'http',
          },
          {
            title: 'TLS Handshake',
            description: 'How HTTPS establishes an encrypted connection',
            tag: 'networking',
          },
        ],
      },
        ...
        topics: [
          {
            ...
            description: 'Name grid regions with template-areas for readable layouts',
            tag: 'cs',
          },
          {
            title: 'Flexbox Alignment',
            description: 'justify-content, align-items, align-self deep dive',
            tag: 'cs',
          },
          {
            title: 'Container Queries',
            description: 'Respond to parent container size, not viewport',
            tag: 'cs',
          },
        ],
      },
