...
export function useAnalytics() {
  ...
  const updateStats = (addTimeMs: number = 0) => {
    setAnalytics((prev: AnalyticsData) => {
      ...
      const newData: AnalyticsData = {
        ...
        userStats: {
          ...
          lastSessionAt:
            addTimeMs > 0 ? Date.now() : prev.userStats.lastSessionAt,
  ...
  const trackSignedUpCerts = (certIds: string[]) => {
    analyticsLogger.debug("trackSignedUpCerts called", { certIds });
    setAnalytics((prev: AnalyticsData) => {
      const newData: AnalyticsData = {
        ...prev,
        signedUpTechCerts: [
          ...new Set([...(prev.signedUpTechCerts ?? []), ...certIds]),
        ],
        updatedAt: Date.now(),
      };
  ...
  const trackSignedUpTechChannels = (techIds: string[]) => {
    analyticsLogger.debug("trackSignedUpTechChannels called", { techIds });
    setAnalytics((prev: AnalyticsData) => {
      const newData: AnalyticsData = {
        ...prev,
        signedUpTechChannels: [
          ...new Set([...(prev.signedUpTechChannels ?? []), ...techIds]),
        ],
        updatedAt: Date.now(),
      };
  ...
  const trackExamAttempt = (attempt: Omit<ExamAttempt, "id" | "timestamp">) => {
    setAnalytics((prev: AnalyticsData) => {
  ...
  const updateCodingProgress = (
    ...
    setAnalytics((prev: AnalyticsData) => {
      ...
      const newProgress: CodingChallengeProgress = {
        ...
        completedAt:
          status === "completed" ? Date.now() : (existing?.completedAt ?? null),
      };
      const newData: AnalyticsData = {
        ...prev,
        codingProgress: {
  ...
  const trackVoicePractice = (
    promptId: string,
    channelId: string,
    rating: number | null,
  ) => {
    setAnalytics((prev: AnalyticsData) => {
      const entry: VoicePracticeEntry = {
        promptId,
        channelId,
        rating,
      ...
      // Apply data retention and max limits
  ...
  const trackQAAnswered = (questionId: string) => {
    setAnalytics((prev: AnalyticsData) => {
      const newProgress: QAProgress = {
        questionId,
        answered: true,
        answeredAt: Date.now(),
      };
  ...
  const trackSearchOpened = () => {
    setAnalytics((prev: AnalyticsData) => {
      ...
      // Apply max limit for search events
      const newSearchEvents = [...(prev.searchEvents ?? []), event].slice(
