...
// Basic stats
const totalRecords = db.prepare('SELECT COUNT(*) as count FROM generated_content').get()
...
svgRecords.forEach(record => {
  try {
    ...
    const hasMermaid = dataStr.includes('mermaid') || dataStr.includes('Mermaid')
    const hasPlantUml = dataStr.includes('plantuml') || dataStr.includes('PlantUML')
...
Object.entries(svgByHash).forEach(([hash, records]) => {
  if (records.length > 1) {
    ...
    records.forEach((r, i) => {
      ...
      console.log(`   ${i === 0 ? '✅' : '🗑️'} ${r.id}${keep} - ${r.channel_id}/${r.content_type}`)
...
const allRecords = db
  .prepare('SELECT id, channel_id, content_type, data, created_at FROM generated_content')
  .all()
const dataHashes = {}
