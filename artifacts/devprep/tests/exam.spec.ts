...
test.describe('Mock Exam Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await bypassOnboarding(page, ['javascript', 'aws-saa', 'cka'], { section: 'exam', channelId: 'javascript' })
    await page.goto('/')
    await waitForAppReady(page)
    await page.click('[data-testid="section-tab-exam"]')
  ...
  test('Start Exam button is clickable', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Submit').or(page.locator('[aria-label="Flag question"]')).or(page.locator('text=00:')),
    ).toBeVisible({ timeout: 8000 })
  })
  ...
  test('can navigate to next question', async ({ page }) => {
    ...
    const nextBtn = page.locator('[aria-label="Next question"]').or(
      page.locator('button:has([data-lucide="chevron-right"])').last(),
    )
