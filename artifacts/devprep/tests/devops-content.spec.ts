 ...
 * Tests the DevOps channel content by navigating the UI.
 */
...
test.describe('DevOps Channel — Content', () => {
  ...
  test('DevOps channel is visible in sidebar', async ({ page }) => {
    await expect(page.locator('button:has-text("DevOps")')).toBeVisible()
  })

  test('switching to DevOps channel updates header', async ({ page }) => {
    const devopsTab = page.locator('button:has-text("DevOps")').first()
